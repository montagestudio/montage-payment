        ...
        gateway.customer.create({
            ...
            */
            ...
            if (err) {
                ...
                if (result.success) {
                    console.info('[Offers] Created customer', customerDetails);
        ...
        gateway.customer.update(customerDetails.id, {
            ...
            */
            ...
            if (err) {
                ...
                if (result.success) {
                    console.info('[Offers] Updated customer', customerDetails);
                    deferred.resolve(result.customer);
                } else {
                    deferred.reject(result);
                }
...
function voidTransaction(transaction, gateway) {
    var deferred = q.defer();
    try {
        console.info("[Offers] Void transation " + transaction.id);
...
function cancelSubscription(subscription, gateway) {
    var deferred = q.defer();
    try {
        console.info("[Offers] Cancel subscription " + subscription.id);
...
function syncCustomerOrderTransaction(customerId, order, transaction, orders, gateway) {
    ...
    if (!existingTransaction) {
        ...
        if (transaction.subscriptionId) {
            ...
            return getSubscription(transaction.subscriptionId, gateway).then(function(subscription) {
                ...
                if (!existingSubscription) {
                    console.log('[Offers] Add missing transaction subscription', transaction.id, subscription.id);
                    ...
                    console.log('[Offers] Update transaction subscription', transaction.id, subscription.id);
            ...
            console.log('[Offers] Update order', order.orderId);
            return order.save();
            // Invalid transations
        } else {

            console.log('[Offers] Void order transaction', transaction.id);
        ...
        console.log('[Offers] Update order', order.orderId);
        console.log('[Offers] Update transaction', transaction.id);
...
function syncCustomerSubscription(customerId, subscription, orders, gateway) {
    ...
    if (!filterByProperty(order.orderId, orders, 'orderId')) {
        console.log('[Offers] Add order', order.orderId);
    ...
    if (subscription.transactions.length === 0) {
        ...
        if (!existingSubscription) {
            console.log('[Offers] Add missing subscription', subscription.id);
            order.subscriptions.push(subscription);
            order.markModified('subscriptions');
            return order.save();
            ...
            console.log('[Offers] Update subscription', subscription.id);
            Object.assign(existingSubscription, subscription);
            order.markModified('subscriptions');
            return order.save();
        } else {
...
function getOrderOfferInfo(order) {
    var planInfo = {
...
function getCustomerPlans(customerId, gateway, useGateway, config) {
...
function getCustomerOffers(customerId, gateway, useGateway, config) {
    return getCustomerOrders(customerId, gateway, useGateway, config).then(function(orders) {
        return orders;
    });
}

function getCustomerOffer(customerId, offerId, gateway, useGateway, config) {
    return getOffer(offerId).then(function(offer) {
        return getCustomerOrders(customerId, gateway, useGateway, config).then(function(orders) {
            ...
            if (offerInfos) {
                return q.resolve(offerInfos);
            } else {
                return q.reject(new Error('OfferNotFound'));
            }
        });
    });
}
...
function sendTransactionReceiptEmail(order, customer, transaction, config) {
    ...
    return MailService.sendEmailTemplate(templateName, templateData, emailObj, config, emailConfig);
}
...
module.exports = {
    ...
    customer: {
        ...
        getOffers: getCustomerOffers,
        getOffer: getCustomerOffer,
        getPlans: getCustomerPlans,
    ...
    order: {
        ...
        getOfferInfo: getOrderOfferInfo,
        void: voidOrder
    },
    subscription: {
        get: getSubscription,
